...
public class GlobalExceptionHandler {
     ...
     * 新增：资源不存在异常处理（如ID不存在）
     * @param e 自定义业务异常
     * @return 标准化响应（用404相关状态码更合理）
     */
    @ExceptionHandler(value = ResourceNotFoundException.class)
    public JsonVO<String> resourceNotFoundHandler(ResourceNotFoundException e) {
        // 若你的 ResultStatus 中有 NOT_FOUND（404）状态，优先用这个；没有就用 PARAMS_INVALID 或自定义
        return JsonVO.create(e.getMessage(), ResultStatus.FAIL);
        // 临时替代方案（如果没有 NOT_FOUND）：
