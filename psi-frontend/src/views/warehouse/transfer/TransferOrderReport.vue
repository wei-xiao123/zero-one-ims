...
const tableData = ref<RowData[]>([
  {
    ...
    documentNumber: 'DBD251031154035',
    totalCost: 23,
    expense: 0,
    contactPerson: '管理员',
    checkStatus: '已审核',
  ...
  },
  {
    ...
    documentNumber: 'DBD251030206032',
    totalCost: 8,
    expense: 0,
    contactPerson: '管理员',
    checkStatus: '未审核',
...
const onImportData = (rows: any[]) => {
  const mapped = rows.map((r, i) => ({
    ...
    documentNumber: r.documentNumber ?? r.单据编号 ?? 'DBD' + Date.now() + i,
    ...
    details: [
      {
        ...
        unit: r.unit ?? r.单位 ?? '',
        attr: r.attr ?? r.辅助属性 ?? '',
        outDepot: r.outDepot ?? r.调出仓库 ?? '',
        inDepot: r.inDepot ?? r.调入仓库 ?? '',
...
function handleSaveDetail() {
  ...
  const saveData = {
    ...
    tableData: detailTableData.value.filter((row) => row.productName), // 只保存有商品数据的行
    isAudited: true
  }
