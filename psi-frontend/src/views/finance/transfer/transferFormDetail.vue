<template>
  ...
  const examine = async () => {
    try {
      ...
      ElMessage.success(`${action}转账单成功!`)
      emit('examined')
    } catch (error) {
      ElMessage.error('[ ERROR ] 服务器响应超时!')
    }
  ...
  const loadExistingData = async () => {
    try {
      // 实际接口调用代码
      // const result = await useHttp().get<any>('service/transferRecord', { parm: props.id })
      ...
      if (props.id) {
        ...
        if (props.id === 1) {
          ...
          Object.assign(form, {
            ...
            number: 'ZZD' + new Date().toISOString().replace(/[-:T.Z]/g, '').slice(2, 15),
            total: 0,
            peopleName: '',
            people: null,
            data: '',
