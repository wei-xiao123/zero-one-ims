    ...
    <div class="header-actions">
      ...
      <div class="right-actions">
        ...
        <ActionButtons
          ...
          :show-batch="true"
          :show-refresh="true"
          export-file-name="其它收入单报表"
          :table-columns="tableColumns"
          template-file-name="其它收入单导入模板"
          @batch="handleBatch"
...
const tableColumns = ref<TableColumn[]>([
  ...
  { prop: 'receiptAmount', label: '单据收款', width: '100', align: 'right', sortable: true },
  ...
  { prop: 'remark', label: '备注信息', width: '150', align: 'left', showOverflowTooltip: true },
  {
    ...
    align: 'center',
    slot: 'operation',
    fixed: 'right' as const // 修复：使用类型断言
  }
])
...
const totalReceiptAmount = computed(() => {
  return tableData.value
    .reduce((sum: number, item: TableRow) => sum + (item.receiptAmount || 0), 0)
    .toFixed(2)
})
...
const generateAllMockData = (): TableRow[] => {
  return Array.from({ length: 150 }, (_, index) => ({
    ...
    billNo: `IN${String(index + 1001).padStart(4, '0')}`,
    billAmount: Math.random() * 10000 + 1000,
    actualAmount: Math.random() * 10000 + 1000,
    receiptAmount: Math.random() * 10000 + 1000,
...
const filterDataBySearchParams = (data: TableRow[], params: SearchParams): TableRow[] => {
  ...
  if (params.customer && params.customer.trim() !== '') {
    filteredData = filteredData.filter((item) =>
      item.customer.toLowerCase().includes(params.customer!.toLowerCase())
    )
  }
...
const fetchTableData = async (searchParams: SearchParams = {}) => {
  loading.value = true
  try {
    const response: ApiResponse = await mockFetchOtherIncomeData({
...
// 模拟数据获取函数
const mockFetchOtherIncomeData = (params: SearchParams): Promise<ApiResponse> => {
...
const handleExport = () => {
  const exportData = tableData.value.map((item) => ({
    ...
    单据收款: item.receiptAmount.toFixed(2),
