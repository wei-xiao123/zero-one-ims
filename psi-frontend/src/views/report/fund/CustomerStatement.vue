<template>
  <div class="customer-statement area">
    <div class="layout">
 ...
 * 导出相关数据
 */
const exportFileName = ref('客户对账单')
...
function useMockData() {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    const row: TableRow = {
      ...
      discount: Math.random() * 8000,
      actual: Math.random() * 142000,
      money: Math.random() * 130000,
      ...
      for (let j = 1; j <= 3; j++) {
        details.push({
          ...
          detail: {
            ...
            tat: Math.random() * 5000,
            tpt: Math.random() * 55000
          },
          total: 0,
          discount: 0,
          actual: 0,
...
async function loadData() {
  ...
  try {
    ...
    // TODO: 替换为实际的 API 地址
    const response = await axios.post('/api/crt/cct', params)
...
function handleExport() {
  try {
    ...
    import('xlsx').then(({ utils, writeFileXLSX }) => {
      ...
      utils.book_append_sheet(wb, ws, '客户对账单')
