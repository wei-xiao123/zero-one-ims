 ...
 * 导出相关数据
 */
const exportFileName = ref('应付款明细表')
const exportDataList = computed(() => {
  const flattenData = (rows: TableRow[], level = 0): any[] => {
    const result: any[] = []
    rows.forEach(row => {
      result.push({
        '供应商': level > 0 ? `${'  '.repeat(level)}${row.name}` : row.name,
        '单据类型': row.bill || '',
...
function useMockData() {
  ...
  const billTypes = ['采购单', '采购退货单', '付款单', '预付款单']
  const mockData: TableRow[] = []
  
  // 为每个供应商生成数据
  suppliers.forEach((supplierName, supplierIndex) => {
    const supplierKey = `supplier_${supplierIndex}`
    const supplierNode: TableRow[] = []
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      supplierNode.push({
        ...
        number: `BL${20240000 + supplierIndex * 10 + i}`,
        cw: billType === '采购单' ? Math.random() * 50000 : 0,
        pia: billType === '预付款单' ? Math.random() * 20000 : 0,
        balance: 30000 + Math.random() * 100000,
        data: `备注信息${i}`
      })
    }
    ...
    mockData.push({
      ...
      time: '',
      number: '',
      cw: supplierNode.reduce((sum, item) => sum + item.cw, 0),
      pia: supplierNode.reduce((sum, item) => sum + item.pia, 0),
...
async function loadData(page?: number) {
  ...
  try {
    ...
    // TODO: 替换为实际的 API 地址
    const response = await axios.post('/api/crt/cps', params)
...
function handleExport() {
  try {
    ...
    import('xlsx').then(({ utils, writeFileXLSX }) => {
      ...
      utils.book_append_sheet(wb, ws, '应付款明细表')
