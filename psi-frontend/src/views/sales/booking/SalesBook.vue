      ...
      <div class="header-row">
        ...
        <div class="action-buttons-container">
          ...
          <ButtonGroup v-if="!isAudited" align="right" gap="0px">
            ...
            <!-- 使用 el-button 替代 BusinessButton 的审核按钮 -->
            <el-button
              :loading="auditing"
              @click="handleAudit"
              type="primary"
 ...
 * 导入销售订单数据
 */
const importSaleOrders = async (file: File) => {
  try {
    const response = await salesBookApi.importSaleOrders(file)
    if (response.code === 10000) {
      ElMessage.success('✅ 导入成功')
      // 导入成功后，如果需要重新加载当前订单数据，检查是否有当前订单ID
      if (currentOrderId.value) {
        loadOrderData(currentOrderId.value)
      } else {
        // 如果没有当前订单，可以刷新界面或重置表单
        handleRefresh()
      }
    } else {
...
const exportDetailReport = async () => {
  if (!currentOrderId.value) {
    ElMessage.warning('请先保存订单再导出')
    return
  }

  try {
    const response = await salesBookApi.exportDetailReport([currentOrderId.value])
    // 处理文件下载
    downloadFile(response, '销售订单详细报表.xlsx')
...
const exportSimpleReport = async () => {
  ...
  try {
    const response = await salesBookApi.exportSimpleReport([currentOrderId.value])
    // 处理文件下载
    downloadFile(response, '销售订单简单报表.xlsx')
    ElMessage.success('✅ 导出成功')
  } catch (error) {
    console.error('❌ 导出失败:', error)
    ElMessage.error('导出失败')
...
const loadOrderData = async (orderId: string) => {
  try {
    ...
    if (listResponse.code === 10000 && listResponse.data?.rows && listResponse.data.rows.length > 0) {
...
const onGridReady = (params: GridReadyEvent) => {
  ...
  if (route.query.dataTransfer === 'true' && params.api) {
    ...
    if (transferData) {
      console.log('📥 [SalesBook] 接收到传输数据:', transferData)
      console.log('  - 来源:', transferData.from)
      console.log('  - 数据条数:', transferData.tableData?.length || 0)
      console.log('  - 时间戳:', transferData.timestamp)

      // 填充表头数据
      const transferDataAny = transferData as any
      if (transferDataAny.customer || transferDataAny.supplier) {
        headerForm.customer = transferDataAny.customer || transferDataAny.supplier || ''
      }
  ...
  if (savedData && params.api) {
    console.log('🔄 [SalesBook] 检测到已保存的数据，恢复中...')
    console.log('  - 数据条数:', savedData.tableData?.length || 0)

    // 恢复表头数据
    const savedHeaderForm = savedData.headerForm as any
    if (savedHeaderForm.customer || savedHeaderForm.supplier) {
      headerForm.customer = savedHeaderForm.customer || savedHeaderForm.supplier || ''
    }
    ...
    if (
      ...
      console.log('✅ [SalesBook] 已恢复 ' + restoredRows.length + ' 条数据')
      // 静默恢复数据，不显示提示信息
    } else {
      // 如果没有表格数据，添加一行空白
      addGridRow(params.api)
