...
import { PurchaseOrderAPI, type PurchaseOrderAddDTO } from '@/apis/purchase/PurchaseBooking/order'
...
function handleSupplierSearch(params: any) {
  if (params && typeof params === 'object') {
    ...
    const id = params.id || params.supplierId || null
    headerForm.supplier = name || ''
    // 临时方案：开发阶段，如果没有ID但有名称，生成临时ID用于联调
    supplierId.value = id || (name ? `temp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : null)
    // 如果只有名称没有ID，尝试根据名称查找ID（这里需要根据实际情况调整）
    if (!supplierId.value && name) {
      console.warn('供应商选择未返回ID，可能需要根据名称查找ID')
    }
  }
...
function onEditableBlur(row: any, field: EditableField, e: Event) {
  ...
  if (field === 'remark') {
    row.remark = text
    // 备注字段变化时也需要同步到 store（虽然不影响金额计算）
    syncTableDataToStore()
    return
  }
...
function handleWarehouseConfirm(selected: any) {
  ...
  // 保存仓库ID（用于提交到后端）
  row.warehouseId = selected.id || selected.warehouseId || ''
...
function handleBatchWarehouseSelect(selected: any) {
  ...
  const selectedId = selected.id || selected.warehouseId || ''
...
const onGridReady = (params: GridReadyEvent) => {
  ...
  if (route.query.dataTransfer === 'true' && params.api) {
    ...
    if (transferData) {
      console.log('📥 [PurchaseBook] 接收到传输数据:', transferData)
      ...
      if (transferData.tableData && Array.isArray(transferData.tableData)) {
        ...
        const fromText = transferData.from === 'purchase-order' ? '采购单' : '其他页面'
      ...
      console.warn('⚠️ Store 中没有找到传输数据，可能已过期或未设置')
      // 初始化时添加一行空白数据
      addGridRow(params.api)
    }
    return
  ...
  const savedData = purchaseStore.getPurchaseBookData
  if (savedData && params.api) {
    console.log('🔄 [PurchaseBook] 检测到已保存的数据，恢复中...')
    ...
    if (
      ...
      console.log('✅ [PurchaseBook] 已恢复 ' + restoredRows.length + ' 条数据')
      // 静默恢复数据，不显示提示信息
    } else {
      // 如果没有表格数据，添加一行空白
      addGridRow(params.api)
  ...
  // 优先级3: 正常初始化：添加一行空白数据
  if (params.api) {
    console.log('🆕 [PurchaseBook] 初始化空白表格')
    addGridRow(params.api)
  }
}
...
/**
 * 同步 el-table 数据到 Pinia store
 * 用于实时更新底部表单的单据金额
 */
const syncTableDataToStore = () => {
  // 只同步有效的商品行（有商品名称的行）
  const validRows = tableData.value
    .filter((row) => row.productName && row.productName.trim() !== '')
    .map((row) => ({
      tpt: Number(row.total) || 0, // 价税合计，DocumentForm 需要这个字段
      // 保留其他字段以便后续使用
      key: row.key || row.goodsId || null,
      name: row.productName,
      number: row.productCode,
      spec: row.spec,
      attr: row.attr,
      unit: row.unit,
      ...
      nums: Number(row.quantity) || 0,
      total: Number(row.amount) || 0,
      tax: Number(row.taxRate) || 0,
      tat: Number(row.taxAmount) || 0,
      data: row.remark || ''
    }))
  
  purchaseStore.setItems(validRows)
  console.log('同步 el-table 数据到 store，有效商品数:', validRows.length)
}
...
const onQuickEnter = (value: string, rowIndex: number) => {
  ...
  // 示例：调用 API 查询商品
  // searchGoods(value).then(goods => {
  //   if (goods) {
  //     const node = grid.options.api.getDisplayedRowAtIndex(rowIndex)
  //     if (node) {
  //       node.setData({
  //         ...node.data,
  //         key: goods.id,
...
const handleProductPopupClose = () => {
  ...
  showProductPopup.value = false
  // 注意：不在这里重置 currentRowIndex，因为 confirm 事件处理完后会重置
}
...
async function handleSave() {
  ...
  if (!documentFormData.arrivalDate) {
    ElMessage.warning('请选择到货日期')
    return
  }

  if (!documentFormData.actualAmount) {
    ElMessage.warning('请输入实际金额')
    return
  }

  // 辅助函数：将 ID 转换为字符串类型
  const convertToStringId = (id: any, fieldName: string = 'ID'): string => {
    if (id === null || id === undefined) {
      throw new Error(`${fieldName}值不能为空`)
    }
    if (typeof id === 'number') {
      if (id === 0) {
        throw new Error(`${fieldName}值不能为0`)
      }
      return String(id)
    }
    if (typeof id === 'string') {
      if (id.trim() === '') {
        throw new Error(`${fieldName}值不能为空字符串`)
      }
      // 验证是否为有效的数字字符串
      const num = Number(id)
      if (!isNaN(num) && num !== 0) {
      ...
      // 如果字符串不是纯数字，可能是仓库编号等，提示用户
      throw new Error(`${fieldName}值格式错误: "${id}"。请确保选择的是有效的${fieldName}，而不是编号或名称`)
    }
    if (id && typeof id === 'object') {
      const numId = id.id || id.key
      if (numId) {
        const num = Number(numId)
        if (!isNaN(num) && num !== 0) {
          return String(numId)
        }
      }
    }
    throw new Error(`无效的${fieldName}值: ${id}`)
  }

  // 辅助函数：将 ID 转换为数字类型（用于 warehouse 等需要数字类型的字段）
  const convertToNumberId = (id: any, fieldName: string = 'ID'): number => {
    if (id === null || id === undefined) {
      throw new Error(`${fieldName}值不能为空`)
    }
    ...
    if (typeof id === 'number') {
      ...
      const objId = id.id || id.key
      if (objId) {
        const num = Number(objId)
        if (!isNaN(num) && num !== 0) {
          numId = num
        }
      }
    ...
    // 检查是否超出 Integer 范围（-2147483648 到 2147483647）
    const MAX_INTEGER = 2147483647
    const MIN_INTEGER = -2147483648
    if (numId > MAX_INTEGER || numId < MIN_INTEGER) {
      throw new Error(
        `${fieldName}值 ${numId} 超出了后端可处理的范围（-2147483648 到 2147483647）。` +
        `这可能是数据问题，请选择其他${fieldName}或联系系统管理员。`
      )
    }
  ...
  // 转换表格数据为后端需要的格式
  const purchaseOrderInfoDTOList = validTableData.map((row: any, index: number) => {
    // 根据实际使用的表格类型，字段名可能不同
    ...
    const goods = convertToStringId(goodsId, '商品ID')
    
    // 将 warehouse ID 转换为字符串类型（后端期望 string 类型）
    // 优先使用 warehouseId，如果不存在则报错（不能使用仓库名称）
    if (!row.warehouseId) {
      throw new Error(`第 ${index + 1} 行的仓库ID不能为空，请选择仓库`)
    }
    const warehouse = convertToStringId(row.warehouseId, '仓库ID')
    ...
    return {
      attr: row.attr || '',
      data: row.remark || row.data || '',
      discount: row.discount || 0,
      dsc: row.dsc || 0,
      goods: goods, // 商品ID，字符串类型
      nums: isElTable ? (Number(row.quantity) || 0) : (Number(row.nums) || 0),
      price: Number(row.price) || 0,
      tat: isElTable ? (Number(row.taxAmount) || 0) : (Number(row.tat) || 0),
      tax: isElTable ? (Number(row.taxRate) || 0) : (Number(row.tax) || 0),
      total: isElTable ? (Number(row.amount) || 0) : (Number(row.total) || 0),
      tpt: isElTable ? (Number(row.total) || 0) : (Number(row.tpt) || 0),
      unit: unit, // 确保是字符串类型
      warehouse: warehouse // 仓库ID，字符串类型（后端期望 string）
    }
  })

  // 构建请求数据
  const requestData: PurchaseOrderAddDTO = {
    actual: Number(documentFormData.actualAmount) || 0,
    arrival: documentFormData.arrivalDate || headerForm.documentDate,
    data: documentFormData.remarks || '',
    file: '', // 单据附件，需要处理文件上传
    logistics: documentFormData.logistics 
      ? (typeof documentFormData.logistics === 'string' 
          ? documentFormData.logistics 
          : documentFormData.logistics.number || '')
      : '',
    number: headerForm.documentNumber || '',
    people: convertToStringId(documentFormData.relatedPerson, '关联人员ID'), // 关联人员ID，字符串类型
    purchaseOrderInfoDTOList: purchaseOrderInfoDTOList,
    source: undefined, // 关联单据ID，如果有的话，需要转换为字符串
    supplier: convertToStringId(supplierId.value, '供应商ID'), // 供应商ID，字符串类型
  ...
  try {
    // 调用后端接口
    const response = await PurchaseOrderAPI.addPurchaseOrder(requestData)
    
    // 打印响应数据，方便调试
    console.log('采购订单响应数据:', {
      code: response.code,
    ...
    if (response.code === 10000) {
      // 保存订单ID（从响应中获取，如果响应data是订单ID）
      if (response.data) {
        orderId.value = response.data
        console.log('✅ 订单ID已保存:', orderId.value)
      }
      ...
      ElMessage.success(`保存成功，已保存 ${validTableData.length} 条数据`)
    } else {
      // 打印错误响应详情
      console.error('采购订单保存失败:', {
        code: response.code,
        message: response.message,
        data: response.data,
        fullResponse: response
...
async function handleGeneratePurchaseOrder() {
  ...
  const validTableData = allTableData.filter((item) => {
    const goodsId = item.key || item.goodsId
    const warehouseId = item.warehouseId
    return goodsId && warehouseId
  })
  
  if (validTableData.length === 0) {
    ElMessage.warning('当前订单没有有效的明细行数据，请先添加商品明细后再生成采购单')
    return
  }
  ...
  try {
    ...
    if (response.code === 10000 && response.data) {
      ...
      const transferData = {
        ...
        supplier: otherData?.supplier || form.supplier || headerForm.supplier,
        documentDate: otherData?.time || headerForm.documentDate,
        documentNumber: otherData?.number || headerForm.documentNumber,
