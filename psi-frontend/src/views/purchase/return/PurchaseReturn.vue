<template>
  <div class="page-container">
    ...
    <!-- 底部表单 -->
    <DocumentForm :fields="purchaseReturnFields" />
...
const headerForm = reactive({
  ...
  documentNumber: 'CGTH2510262244385'
})
...
const onGridReady = (params: GridReadyEvent) => {
  ...
  if (route.query.dataTransfer === 'true' && params.api) {
    ...
    if (transferData) {
      console.log('📥 [PurchaseReturn] 接收到传输数据:', transferData)
  ...
  const savedData = purchaseStore.getPurchaseReturnData
  if (savedData && params.api) {
    console.log('🔄 [PurchaseReturn] 检测到已保存的数据，恢复中...')
    ...
    if (
      ...
      console.log('✅ [PurchaseReturn] 已恢复 ' + restoredRows.length + ' 条数据')
      // 静默恢复数据，不显示提示信息
    } else {
      // 如果没有表格数据，添加一行空白
      addGridRow(params.api)
  ...
  // 优先级3: 正常初始化：添加一行空白数据
  if (params.api) {
    console.log('🆕 [PurchaseReturn] 初始化空白表格')
    addGridRow(params.api)
  }
}
...
function handleSave() {
  ...
  // 保存到 Pinia store
  purchaseStore.savePurchaseReturnData({
    headerForm: {
  ...
  console.log('💾 已保存到 store:', tableData.length, '条数据')
