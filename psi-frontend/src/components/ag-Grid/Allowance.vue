<template>
	<div class="allowance">
		<div class="title">
			<span>{{ title }} </span>
			<el-popover popper-class="allowancePopper" trigger="click" v-model:visible="visible" @show="onShow">
				<template #reference>
					<i class="el-icon-sort"></i>
				</template>
				<el-input ref="taxInput" v-model="value" @keydown.enter="save" placeholder="折扣率" size="small" clearable>
					<template #append>
...
<script setup>
	/**
	 * Header Component | 批量折扣率 (Vue 3)
	 * 用途：一次性为当前 ag-Grid 网格的所有有效行设置折扣率。
	...
	function save() {
		...
		if (lib.validate('percentage', value.value)) {
			...
			ElMessage({ type: 'warning', message: '折扣率不正确!' })
		}
	}
